...
import { LayoutGrid, Settings, LogOut, ChevronLeft, ChevronRight, User as UserIcon, Home, Lightbulb, FileText, Library, TrendingUp, Users, Music } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, user }) => {
  ...
  const navItems = [
    ...
    { id: 'marketplace', label: 'Marketplace', icon: Music }
  ];

  return (
    <div className={`relative ${isCollapsed ? 'w-20' : 'w-52'} bg-[#010208] border-r border-white/5 flex flex-col h-full flex-shrink-0 transition-all duration-300 z-50`}>
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        title={isCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
        className="absolute top-[26px] -right-3.5 w-7 h-7 bg-[#010208] border border-white/10 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 transition-colors z-50"
      >
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center transition-all duration-200 focus:outline-none focus:text-pink-500 active:text-pink-500 group ${
                isActive
                  ? 'text-pink-500 font-bold'
                  : 'text-white/60 hover:text-pink-400'
              } ${isCollapsed ? 'py-3 justify-center w-12 mx-auto' : 'w-full py-2.5'}`}
            >
              {isCollapsed ? (
                <Icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-pink-500' : 'group-hover:text-pink-400'}`} />
              ) : (
                <div className="w-36 mx-auto flex items-center justify-start gap-3">
                  <Icon className={`w-4 h-4 shrink-0 transition-colors ${isActive ? 'text-pink-500' : 'group-hover:text-pink-400'}`} />
                  <span className="text-sm font-semibold tracking-wide transition-colors">{item.label}</span>
                </div>
              )}
            </button>
          );
      ...
      <div className={`p-4 mt-auto border-t border-white/5 flex flex-col gap-3 ${isCollapsed ? 'items-center' : ''}`} ref={menuRef}>
        <div className="relative w-full">
          {isMenuOpen && (
            <div className={`absolute bottom-full mb-2 bg-[#010208] border border-white/10 rounded-xl shadow-lg shadow-black/50 overflow-hidden flex flex-col w-48 ${isCollapsed ? 'left-10' : 'left-0'}`}>
              <button
                onClick={() => {
                  setView('settings');
                  setIsMenuOpen(false);
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-500 to-pink-600 flex items-center justify-center overflow-hidden shrink-0">
               {user?.photoURL ? (
            ...
            {!isCollapsed && (
              <div className="flex flex-col overflow-hidden text-left flex-1">
                 ...
                 <span className="text-[9px] font-semibold truncate text-white leading-tight">
                 ...
                 <div className="mt-2 w-full">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">AI Credits</span>
                      <span className="text-[8px] font-bold text-pink-400">{credits !== null ? credits : '--'}/50</span>
                    </div>
